...
export function MobileShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-dvh flex-1 items-center justify-center bg-background sm:bg-zinc-200 sm:p-6 sm:dark:bg-zinc-950">
      <div className="relative h-dvh w-full sm:h-[min(92dvh,900px)] sm:w-auto sm:aspect-[1206/2622] sm:rounded-[3.25rem] sm:border-[6px] sm:border-zinc-900 sm:bg-zinc-900 sm:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.4)] sm:dark:border-zinc-800">
        <div className="relative flex h-full w-full transform-gpu flex-col overflow-hidden bg-background sm:rounded-[2.75rem]">
          <AmbientPaletteProvider>
            ...
            <div className="relative z-10 flex flex-1 flex-col overflow-y-auto pt-[env(safe-area-inset-top)] sm:pt-6">
              {children}
            </div>
          </AmbientPaletteProvider>
        </div>
